...
public class ProblemService {
	...
	public Problems create(String json)  {
		try {
			...
			Optional<Problems> problemoptional = problemsRepository.findById(problemId);
			if(problemoptional.isEmpty()) {
				Problems insert = new Problems();
				insert.setProblemId(problemId);
				insert.setEventCase(eventCase);
				insert.setRoom(room);
				insert.setContent(content);
				insert.setEventDate(eventDate != null ? new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").parse(eventDate) : null);
				insert.setCloseDate(closeDate != null ? new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").parse(closeDate) : null);
